...
const benchmarkSimpleOperations = () => {
  ...
  for (const size of sizes) {
    ...
    console.log(
      `  Memory after basic ops: ${memAfterBasic.toFixed(2)} MB (diff: ${(memAfterBasic - memBefore).toFixed(2)} MB)`,
    );
    console.log(
      `  Memory after cache ops: ${memAfterFull.toFixed(2)} MB (diff: ${(memAfterFull - memAfterBasic).toFixed(2)} MB)`,
    );
    console.log('');
  }
};
...
const benchmarkInitSpeed = () => {
  ...
  for (const size of sizes) {
    ...
    console.log(
      `  1000 iterations with cache ops: ${(endWithCache - startWithCache).toFixed(2)} ms`,
    );
    console.log('');
  }
};
